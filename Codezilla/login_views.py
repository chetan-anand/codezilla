...
def register():                    #renders the signup.html
	if request.method == 'POST' :
		...
		'''db = get_db()
		db.execute('insert into users (username, pass, email, name) values (?, ?, ?, ?)',[request.form['username'], request.form['password'], request.form['email'], request.form['name']])
		db.commit()'''
		message = "Congratulations "+request.form['username']+", Please check your email to activate the account "
		mailing.confirm_mail(request.form['email'])
